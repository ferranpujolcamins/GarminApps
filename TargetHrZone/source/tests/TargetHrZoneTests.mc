...
function testWhenDefaultValueIsAverage(logger as Logger) as Boolean {
    ...
    var field = new TargetHrZoneField(currentWorkoutStepProvider, userProfileProvider, properties)
        .compute();
...
function testDefaultValueIsNotDisplayedWhenTargetIsExactlyAZone(logger as Logger) as Boolean {
    ...
    var field = new TargetHrZoneField(currentWorkoutStepProvider, userProfileProvider, new PropertiesModule.ApplicationProperties())
        .compute();

    logger.debug("field = " + field);
    return field.equals("4");
}
